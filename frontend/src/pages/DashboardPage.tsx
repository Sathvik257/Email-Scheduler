...
export function DashboardPage({
  ...
  const metrics = useMemo(() => {
    ...
    const completedCount = sentCount + failedCount;
    const successRate =
      completedCount > 0
        ? Math.round((sentCount / completedCount) * 100)
        : 0;
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb]">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <section className="relative overflow-hidden rounded-lg bg-[#082521] px-6 py-8 text-white shadow-2xl shadow-teal-950/20 sm:px-8 lg:px-10">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(45,212,191,0.32),transparent_24rem),radial-gradient(circle_at_92%_12%,rgba(249,115,22,0.24),transparent_22rem)]" />
          <div className="absolute inset-0 opacity-[0.1] [background-image:linear-gradient(#ffffff_1px,transparent_1px),linear-gradient(90deg,#ffffff_1px,transparent_1px)] [background-size:44px_44px]" />

          <div className="relative flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-xs font-bold backdrop-blur">
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-teal-50/75 sm:text-base">
                Schedule persistent email jobs, monitor BullMQ processing,
                manage multiple senders, and review Ethereal delivery previews.
              </p>

              <div className="mt-5 flex flex-wrap items-center gap-3 text-xs font-semibold text-teal-50/90">
                <SystemPill
          ...
          <div className="relative mt-8 grid gap-3 sm:grid-cols-3">
            <HeroStat label="Success rate" value={`${metrics.successRate}%`} />
            <HeroStat label="Active senders" value={String(senders.length)} />
            <HeroStat label="Refresh cadence" value="5s" />
          </div>
        </section>
        ...
        {notice && (
          <div
            className={`mt-5 flex items-start gap-3 rounded-lg border px-4 py-3 text-sm shadow-sm ${
              notice.type === "success"
        ...
        <section className="mt-7 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
          ...
          <div className="flex overflow-x-auto border-b border-slate-200 bg-slate-50/60 px-3 pt-3 sm:px-5">
            <TabButton
...
function MetricCard({
  ...
  return (
    <div className="group rounded-lg border border-slate-200 bg-white p-5 shadow-sm shadow-slate-900/5 transition duration-200 hover:-translate-y-1 hover:border-teal-100 hover:shadow-lg hover:shadow-slate-900/10">
    ...
    <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 backdrop-blur">
      <span
        className={`h-2 w-2 rounded-full ${
          active ? "bg-teal-300" : "bg-rose-300"
        }`}
      />
      {icon}
      <span>{text}</span>
...
function HeroStat({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.08] p-4 backdrop-blur transition hover:bg-white/[0.12]">
      <p className="text-2xl font-black text-white">{value}</p>
      <p className="mt-1 text-xs font-semibold text-teal-50/60">
        {label}
      </p>
    </div>
  );
